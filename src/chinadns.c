...
static int parse_args(int argc, char **argv) {
  ...
  if (dns_servers == NULL) {
    ...
    ERR("No dns server provided.\n");
    exit(1);
  }
  if (chnroute_file == NULL) {
    //dns_servers = strdup(default_dns_servers);
    ERR("No chnroute file provided.\n");
    exit(1);
  }
...
static int resolve_dns_servers() {
  ...
  */
  ...
  if ((has_chn_dns == 0) || (has_trusted_dns == 0)) {
        VERR("You should have at least one Chinese DNS and one trusted DNS.\n");
        return -1;  	
  	}
...
static int test_dns_server_type(struct sockaddr *addr) {
  ...
  if (r) {
    ...
    if (test_ip_in_list(((struct sockaddr_in *)addr)->sin_addr,
      ...
      /*we simply consider any server which is neither china dns nor trusted dns is unaccepted, because it doesn't make any sense.*/
	  VERR("you must specify a china dns\n");
	  exit(1);
      //return FOREIGN_DNS;
    }
